...
import { z } from 'zod'
import { fetchAIKeywordData, fetchLLMMentions, type AIKeywordDataState, type LLMMentionsState } from './action'
...
export default function FormAi() {
  ...
  const [keywordState, keywordFormAction, isKeywordPending] = useActionState(fetchAIKeywordData, initialKeywordState)

  // États pour LLM Mentions
  const initialMentionsState: LLMMentionsState = {
    success: false,
  }
  const [mentionsState, mentionsFormAction, isMentionsPending] = useActionState(fetchLLMMentions, initialMentionsState)
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full min-w-0 overflow-x-hidden">
        <TabsList className="mb-6 grid w-full grid-cols-2 lg:mb-8 lg:w-auto">
          <TabsTrigger
            value="ai-keywords"
            className={`gap-2 lg:px-8 ${
              activeTab === 'ai-keywords' ? 'bg-mist-100 text-mist-600 dark:bg-mist-600 dark:text-mist-100' : ''
            }`}
          >
          ...
          <TabsTrigger
            value="llm-mentions"
            className={`gap-2 lg:px-8 ${
              activeTab === 'llm-mentions' ? 'bg-mist-100 text-mist-600 dark:bg-mist-600 dark:text-mist-100' : ''
            }`}
          >
          ...
          <Card className="border-2 shadow-lg">
            <CardHeader className="lg:px-8 lg:py-7">
              ...
              <CardDescription className="mt-2 lg:text-base">
                Obtenez le volume de recherche, l&apos;intention utilisateur et les tendances d&apos;utilisation dans
                les outils d&apos;IA. Analysez jusqu&apos;à 100 mots-clés simultanément.
              </CardDescription>
            </CardHeader>
            <CardContent className="lg:px-8 lg:pb-8">
              <Form {...keywordForm}>
                <form onSubmit={keywordForm.handleSubmit(onKeywordSubmit)} className="space-y-6">
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          <textarea
                            ...
                            rows={6}
                            className="border-input bg-background ring-offset-background placeholder:text-muted-foreground focus-visible:ring-ring flex min-h-[80px] w-full rounded-md border px-3 py-2 text-sm focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 lg:h-auto lg:text-base"
                          />
                        </FormControl>
                        <FormMessage className="text-red-500" />
                        <p className="text-muted-foreground text-xs">
                          Analyse ciblée pour le marché français (France, langue française). Vous pouvez analyser
                          jusqu&apos;à 100 mots-clés à la fois.
                        </p>
                      </FormItem>
                    )}
                  />
                  ...
                  <Button
                    type="submit"
                    className="h-11 w-full gap-2 hover:cursor-pointer lg:h-12 lg:text-base"
                    disabled={isKeywordPending}
                  >
                    {isKeywordPending ? (
                      <>
              ...
              {keywordState?.error && (
                <div className="mt-6 rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-950/20">
                  ...
                  {typeof keywordState.error === 'string' ? (
                    <p className="mt-1 text-sm text-red-700 dark:text-red-300">{keywordState.error}</p>
                  ) : (
                    <div className="mt-2 space-y-1">
                      {Object.entries(keywordState.error).map(([field, errors]) => (
                        <div key={field}>
                          <p className="text-xs font-medium text-red-800 dark:text-red-300">{field}:</p>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-semibold lg:text-2xl">
                        {keywordState.keywords && keywordState.keywords.length === 1
                      ...
                      {keywordState.keywords && keywordState.keywords.length > 1 && (
                        <div className="mt-3 flex flex-wrap gap-2">
                          {keywordState.keywords.slice(0, 10).map((kw, idx) => (
                    ...
                    <div key={index} className="space-y-6">
                      ...
                      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-2 lg:gap-6">
                          ...
                          <CardContent className="pt-6">
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                <p className="text-muted-foreground text-sm font-medium">Volume de recherche IA</p>
                                <p className="mt-2 text-3xl font-bold text-purple-600 lg:text-3xl dark:text-purple-400">
                                  {item.ai_search_volume?.toLocaleString('fr-FR') || 'N/A'}
                                </p>
                        ...
                        <Card className="border-2-blue-500/20 bg-gradient-to-br from-blue-500/5 to-transparent">
                          <CardContent className="pt-6">
                            <div>
                              <p className="text-muted-foreground text-sm font-medium">Mot clé analysé</p>
                              <div className="mt-3 flex items-center gap-2">
                                <Badge className="px-4 py-2 text-base break-words lg:text-lg" color="blue">
                                  {item.keyword}
                                </Badge>
                              </div>
                              <p className="text-muted-foreground mt-3 text-xs">
                                {item.ai_monthly_searches?.length || 0} mois de données historiques
                              </p>
                            </div>
                          </CardContent>
                        </Card>
                      ...
                      {item.ai_monthly_searches && item.ai_monthly_searches.length > 0 && (
                        <Card className="border-2">
                          ...
                          <CardContent>
                            <div className="space-y-4">
                              {item.ai_monthly_searches.slice(-12).map((monthData, idx) => {
                                const maxVolume = Math.max(...item.ai_monthly_searches.map((m) => m.ai_search_volume))
                                const percentage = (monthData.ai_search_volume / maxVolume) * 100

                                const monthName = new Date(monthData.year, monthData.month - 1).toLocaleDateString(
                                  'fr-FR',
                                  {
                                    month: 'long',
                                    year: 'numeric',
                                  ...
                                  <div key={idx} className="space-y-2">
                                    <div className="flex items-center justify-between text-sm">
                                      ...
                                      <span className="font-bold text-purple-600">
                                        {monthData.ai_search_volume.toLocaleString('fr-FR')}
                                      </span>
                                    </div>
                          ...
                          <ul className="space-y-3">
                            ...
                            <li className="flex items-start gap-3">
                              ...
                              <span className="text-sm lg:text-base">
                                Créez du contenu structuré et détaillé pour être cité par les IA comme source fiable.
                              </span>
                            </li>
                            <li className="flex items-start gap-3">
                              <ChevronRight className="mt-0.5 h-5 w-5 shrink-0 text-amber-600" />
                              <span className="text-sm lg:text-base">
                                Utilisez un balisage sémantique (schema.org) pour améliorer votre visibilité dans les
                                LLM.
                              </span>
                            </li>
                          </ul>
        ...
        {/* Tab: LLM Mentions */}
        <TabsContent value="llm-mentions" className="w-full min-w-0 space-y-6 overflow-x-hidden lg:space-y-8">
          <Card className="w-full min-w-0 overflow-x-hidden border-2 shadow-lg">
            <CardHeader className="w-full min-w-0 overflow-x-hidden lg:px-8 lg:py-7">
              ...
              <CardDescription className="mt-2 lg:text-base">
                Découvrez comment votre marque, domaine ou mots-clés sont mentionnés dans les réponses des LLM (Google
                Gemini, etc.)
              </CardDescription>
            </CardHeader>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select onValueChange={field.onChange} defaultValue={field.value} disabled={isMentionsPending}>
                          <FormControl>
                            <SelectTrigger className="h-11 hover:cursor-pointer lg:h-12 lg:text-base">
                              <SelectValue placeholder="Sélectionner un type" />
                            </SelectTrigger>
                          </FormControl>
                          <SelectContent className="bg-mist-600">
                            <SelectItem value="keyword" className="hover:cursor-pointer hover:bg-mist-500">
                              Mot-clé
                            </SelectItem>
                            <SelectItem value="domain" className="hover:cursor-pointer hover:bg-mist-500">
                              Domaine
                            </SelectItem>
                          </SelectContent>
                        </Select>
                        <FormMessage className="text-red-500" />
                        <p className="text-muted-foreground text-xs">
                          Analyse ciblée pour le marché français (France, langue française) via Google
                        </p>
                      </FormItem>
                    )}
                  />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormMessage className="text-red-500" />
                        <p className="text-muted-foreground text-xs">
                          Pour un domaine, saisissez-le sans https:// ou www. (ex: exemple.com)
                        </p>
                      </FormItem>
                    )}
                  />
              ...
              {mentionsState?.error && (
                <div className="mt-6 rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-950/20">
                  ...
                  {typeof mentionsState.error === 'string' ? (
                    <p className="mt-1 text-sm text-red-700 dark:text-red-300">{mentionsState.error}</p>
                  ) : (
                    <div className="mt-2 space-y-1">
                      {Object.entries(mentionsState.error).map(([field, errors]) => (
                        <div key={field}>
                          <p className="text-xs font-medium text-red-800 dark:text-red-300">{field}:</p>
                          {Array.isArray(errors) &&
                            errors.map((error: string, index: number) => (
                ...
                <div className="mt-8 w-full max-w-full min-w-0 space-y-6 overflow-x-hidden">
                  ...
                  <div className="flex w-full max-w-full min-w-0 flex-col gap-4 sm:flex-row sm:items-center">
                    ...
                    <div className="max-w-full min-w-0 flex-1 overflow-x-hidden">
                      ...
                      <p className="text-muted-foreground text-sm lg:text-base">
                        {mentionsState.result.items_count} mention(s) sur {mentionsState.result.total_count} total
                        trouvé(s)
                      </p>
                    </div>
                  </div>
                  ...
                  {mentionsState.result.items.map((item, index) => (
                    ...
                    >
                          ...
                          <div className="max-w-full min-w-0 flex-1 overflow-x-hidden">
                            ...
                            <div className="mt-2 flex w-full max-w-full min-w-0 flex-wrap gap-2">
                              {item.ai_search_volume && (
                                <Badge color="blue" className="max-w-full shrink-0 gap-1">
                                  <TrendingUp className="h-3 w-3 shrink-0" />
                                  <span className="truncate">
                                    Volume IA: {item.ai_search_volume.toLocaleString('fr-FR')}
                                  </span>
                                </Badge>
                              )}
                              {item.first_response_at && (
                                <Badge color="zinc" className="max-w-full shrink-0 gap-1">
                                  <Calendar className="h-3 w-3 shrink-0" />
                                  <span className="truncate">
                                    Première réponse:{' '}
                                    {new Date(item.first_response_at).toLocaleDateString('fr-FR', {
                                      day: 'numeric',
                                      month: 'short',
                                      year: 'numeric',
                                    })}
                              ...
                              {item.last_response_at && (
                                <Badge color="zinc" className="max-w-full shrink-0 gap-1">
                                  ...
                                  <span className="truncate">
                                    ...
                                    {new Date(item.last_response_at).toLocaleDateString('fr-FR', {
                                      day: 'numeric',
                                      month: 'short',
                                      year: 'numeric',
                                    })}
                      ...
                      <CardContent className="w-full max-w-full min-w-0 space-y-6 overflow-x-hidden pt-6">
                        ...
                        {item.question && (
                          <div className="w-full max-w-full min-w-0 overflow-x-hidden rounded-lg border border-blue-200 bg-blue-50/50 p-4 dark:border-blue-800 dark:bg-blue-950/20">
                            <div className="mb-2 flex items-start gap-2">
                              ...
                              <p className="text-sm font-semibold text-blue-900 dark:text-blue-200">Question posée:</p>
                            </div>
                        ...
                        {item.answer && (
                          <div className="w-full max-w-full min-w-0 overflow-x-hidden">
                            <div className="mb-3 flex items-start gap-2">
                              ...
                              <p className="text-muted-foreground text-sm font-semibold">Réponse du LLM:</p>
                            </div>
                            <div className="border-2-purple-200 w-full max-w-full min-w-0 overflow-x-auto rounded-lg bg-gradient-to-br from-purple-50/50 to-pink-50/50 p-4 dark:border-purple-800 dark:from-purple-950/20 dark:to-pink-950/20">
                              <div
                                className="prose prose-sm dark:prose-invert [&_code]:bg-muted [&_pre]:bg-muted w-full max-w-full min-w-0 overflow-x-auto text-sm lg:text-base [&_*]:max-w-full [&_*]:min-w-0 [&_*]:break-words [&_a]:break-all [&_a]:text-blue-600 [&_a]:underline [&_a]:hover:text-blue-800 [&_blockquote]:my-2 [&_blockquote]:border-l-4 [&_blockquote]:border-purple-300 [&_blockquote]:pl-4 [&_blockquote]:italic [&_code]:rounded [&_code]:px-1 [&_code]:py-0.5 [&_code]:text-xs [&_code]:break-all [&_em]:italic [&_h1]:mt-4 [&_h1]:mb-2 [&_h1]:text-lg [&_h1]:font-bold [&_h1]:break-words [&_h2]:mt-3 [&_h2]:mb-2 [&_h2]:text-base [&_h2]:font-semibold [&_h2]:break-words [&_h3]:mt-2 [&_h3]:mb-1 [&_h3]:text-sm [&_h3]:font-semibold [&_h3]:break-words [&_img]:h-auto [&_img]:max-w-full [&_li]:my-1 [&_li]:break-words [&_ol]:my-2 [&_ol]:ml-4 [&_ol]:list-decimal [&_p]:my-2 [&_p]:leading-relaxed [&_p]:break-words [&_pre]:overflow-x-auto [&_pre]:rounded [&_pre]:p-2 [&_pre]:break-all [&_strong]:font-semibold [&_table]:w-full [&_table]:overflow-x-auto [&_ul]:my-2 [&_ul]:ml-4 [&_ul]:list-disc"
                        ...
                        {item.ai_monthly_searches && item.ai_monthly_searches.length > 0 && (
                          <Card className="border-2-purple-500/20 w-full max-w-full min-w-0 overflow-x-hidden bg-gradient-to-br from-purple-500/5 to-transparent">
                            ...
                            <CardContent className="w-full max-w-full min-w-0 overflow-x-hidden">
                              <div className="w-full max-w-full min-w-0 space-y-3">
                                {item.ai_monthly_searches.slice(-6).map((monthData, idx) => {
                                  const maxVolume = Math.max(
                                    ...(item.ai_monthly_searches || []).map((m) => m.ai_search_volume),
                                  )
                                  const percentage = (monthData.ai_search_volume / maxVolume) * 100
                          ...
                          <div className="w-full max-w-full min-w-0 overflow-x-hidden">
                            ...
                            <div className="grid w-full max-w-full min-w-0 gap-3 sm:grid-cols-2">
                              {item.sources.slice(0, 6).map((source, idx) => (
                                <div
                                  key={idx}
                                  className="group bg-card w-full min-w-0 rounded-lg border p-3 transition-all hover:border-purple-300 hover:shadow-sm"
                                >
                                  <div className="flex w-full min-w-0 gap-3 overflow-x-hidden">
                                    ...
                                    <div className="min-w-0 flex-1 overflow-x-hidden">
                                      {source.title && (
                                        <p className="line-clamp-2 text-sm font-medium break-words">{source.title}</p>
                                      )}
                                      {source.domain && (
                                        <p className="text-muted-foreground mt-1 text-xs break-all">{source.domain}</p>
                                      )}
                                      {source.snippet && (
                                      ...
                                      {source.publication_date && (
                                        <p className="text-muted-foreground mt-1 text-xs">
                                          {new Date(source.publication_date).toLocaleDateString('fr-FR')}
                                        </p>
                                      )}
                                    </div>
                                  </div>
                        ...
                        {item.search_results && item.search_results.length > 0 && (
                          <div className="w-full max-w-full min-w-0 overflow-x-hidden">
                            <div className="mb-3 flex items-center gap-2">
                              ...
                              <p className="text-muted-foreground text-sm font-semibold">
                                Résultats de recherche utilisés ({item.search_results.length}):
                              </p>
                            </div>
                            <div className="max-h-60 w-full max-w-full min-w-0 space-y-2 overflow-x-hidden overflow-y-auto">
                              {item.search_results.slice(0, 10).map((result, idx) => (
                                <div
                                  key={idx}
                                  className="bg-muted/30 w-full min-w-0 overflow-x-hidden rounded-md border p-2 text-xs"
                                >
                                  {result.title && <p className="font-medium break-words">{result.title}</p>}
                                  {result.domain && <p className="text-muted-foreground break-all">{result.domain}</p>}
                                  {result.description && (
                              ...
                              {item.search_results.length > 10 && (
                                <p className="text-muted-foreground text-center text-xs">
                                  +{item.search_results.length - 10} autre(s) résultat(s)
                                </p>
                              )}
                            </div>
                          </div>
                        ...
                        {item.fan_out_queries && item.fan_out_queries.length > 0 && (
                          <div className="w-full max-w-full min-w-0 overflow-x-hidden">
                            <div className="mb-3 flex items-center gap-2">
                              ...
                              <p className="text-muted-foreground text-sm font-semibold">
                                Requêtes liées ({item.fan_out_queries.length}
                                ):
                              </p>
                            </div>
                            <div className="flex w-full max-w-full min-w-0 flex-wrap gap-2">
                              {item.fan_out_queries.map((query, idx) => (
                                <Badge key={idx} color="purple" className="max-w-full text-xs break-words">
                          ...
                          <div className="w-full max-w-full min-w-0 overflow-x-hidden">
                            ...
                            <div className="flex w-full max-w-full min-w-0 flex-wrap gap-2">
                              {item.brand_entities.map((entity, idx) => (
                                <Badge key={idx} color="amber" className="max-w-full break-words">
                                  <span className="truncate">
                                    {entity.title}
                                    {entity.category && (
                                      <span className="ml-1 text-xs opacity-75">({entity.category})</span>
                                    )}
                                  </span>
                                </Badge>
                              ))}
                  ...
                  {mentionsState.result.items_count === 0 && (
                    <Card className="border-2">
                      <CardContent className="pt-6">
                        <p className="text-muted-foreground text-center">Aucune mention trouvée pour cette cible.</p>
                      </CardContent>
                    </Card>
                  )}
                </div>
