...
export function TopPagesContent({ projectId }: Props) {
  ...
  const filteredPages =
    data?.pages
      ?.filter((p: PageData) => {
        // Recherche
        if (searchQuery && !p.path.toLowerCase().includes(searchQuery.toLowerCase())) {
          return false
        }
    ...
    return (
      <Card>
        <CardContent className="py-16 text-center">
          ...
          <p className="text-muted-foreground mt-2 text-sm">Vérifiez que votre site est bien indexé</p>
        </CardContent>
      </Card>
    )
  }
      ...
      <div className="space-y-6">
          ...
          <CardContent>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
              {Object.entries(stats.pagesByType).map(([type, count]) => {
                const Icon = PAGE_TYPE_ICONS[type as keyof typeof PAGE_TYPE_ICONS]
                const label = PAGE_TYPE_LABELS[type as keyof typeof PAGE_TYPE_LABELS]
                const countNumber = typeof count === 'number' ? count : 0
                const percentage = (countNumber / stats.totalPages) * 100
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <Badge color="zinc">{countNumber}</Badge>
                    </div>
                    <div className="text-muted-foreground text-xs">{percentage.toFixed(1)}% du total</div>
                  </div>
                )
              })}
            </div>
        ...
        <Tabs defaultValue="all" className="space-y-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <TabsList className="rounded-lg bg-mist-700 p-1">
              <TabsTrigger
                value="all"
                className="transition-colors data-[state=active]:bg-mist-600 data-[state=active]:text-white"
              >
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-5">
                <div className="space-y-2">
                  ...
                  <Input placeholder="Chemin..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={sortBy}
                    onValueChange={(v: 'clicks' | 'impressions' | 'ctr' | 'position') => setSortBy(v)}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              {(searchQuery || pageTypeFilter !== 'all' || performanceFilter !== 'all') && (
            ...
            <Card className="mb-6 border-green-200 bg-green-50 dark:border-green-900 dark:bg-green-950">
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-green-900 dark:text-green-100">Pages Top 3</CardTitle>
                </div>
                <CardDescription>Pages positionnées dans le top 3 de Google</CardDescription>
              </CardHeader>
            </Card>
            ...
            <Card className="mb-6 border-blue-200 bg-blue-50 dark:border-blue-900 dark:bg-blue-950">
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-blue-900 dark:text-blue-100">Pages en progression</CardTitle>
                </div>
                <CardDescription>Pages avec amélioration significative</CardDescription>
              </CardHeader>
            </Card>
            ...
            <Card className="mb-6 border-red-200 bg-red-50 dark:border-red-900 dark:bg-red-950">
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-red-900 dark:text-red-100">Pages en baisse</CardTitle>
                </div>
                <CardDescription>Pages nécessitant une optimisation</CardDescription>
              </CardHeader>
            </Card>
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center">
          <FileText className="text-muted-foreground/50 mx-auto h-12 w-12" />
          <p className="text-muted-foreground mt-4 text-sm font-medium">Aucune page dans cette catégorie</p>
        </CardContent>
      </Card>
    )
  }
          ...
          <Table>
            ...
            <TableBody>
              {pages.map((page, index) => {
                const Icon = PAGE_TYPE_ICONS[page.pageType as keyof typeof PAGE_TYPE_ICONS]
                return (
                  <TableRow key={page.url}>
                      ...
                      <div className="flex items-center gap-2">
                        <Tooltip>
                          <TooltipTrigger asChild>
                            ...
                            >
                              <span className="max-w-xs truncate">{page.path}</span>
                        ...
                        {page.isTop && <Badge className="bg-green-600 text-xs">Top 3</Badge>}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Icon className="text-muted-foreground h-4 w-4" />
                        <span className="text-muted-foreground text-xs">{PAGE_TYPE_LABELS[page.pageType]}</span>
                      </div>
                    </TableCell>
                    <TableCell className="text-right font-semibold">{page.clicks.toLocaleString('fr-FR')}</TableCell>
                    ...
                    <TableCell className="text-right">
                      <Badge
                        color={page.position <= 3 ? 'green' : page.position <= 10 ? 'blue' : 'zinc'}
                        className={
                          page.position <= 3 ? 'bg-green-600' : page.position <= 10 ? 'bg-blue-600 text-white' : ''
                        }
                      >
                            ...
                            <TooltipTrigger>
                              <div
                                className={`flex items-center gap-1 text-xs font-medium ${
                                  page.clicksChange > 0 ? 'text-green-600' : 'text-red-600'
                                }`}
                              >
                                {page.clicksChange > 0 ? (
